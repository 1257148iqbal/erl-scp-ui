...
export const getTimeDifference = (fromDateTime, toDateTime, dutarionType: 'asHours' | 'asMinutes' | 'asSeconds') => {
  ...
  switch (dutarionType) {
    case 'asHours': {
      ...
      const formattedHoursDuration = Number.isInteger(durationAsHours)
        ? durationAsHours
        : Number(Number.parseFloat(durationAsHours).toFixed(2));
      return formattedHoursDuration;
    }
    case 'asMinutes': {
      const durationAsMinutes = duration.asMinutes();
      const formattedMinutesDuration = Number.isInteger(durationAsMinutes)
        ? durationAsMinutes
        : Number(Number.parseFloat(durationAsMinutes).toFixed(2));
      return formattedMinutesDuration;
    }
    case 'asSeconds': {
      const durationAsSeconds = duration.asSeconds();
      const formattedSecondsDuration = Number.isInteger(durationAsSeconds)
        ? durationAsSeconds
        : Number(Number.parseFloat(durationAsSeconds).toFixed(2));
      return formattedSecondsDuration;
    }

    default: {
      const _durationAsSeconds = duration.asSeconds();
      const _formattedSecondsDuration = Number.isInteger(_durationAsSeconds)
        ? _durationAsSeconds
        : Number(Number.parseFloat(_durationAsSeconds).toFixed(2));
      return _formattedSecondsDuration;
    }
  }
};
