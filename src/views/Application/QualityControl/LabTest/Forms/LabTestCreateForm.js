...
import { CancelButton, Form, FormWrapper, ResetButton, SaveButton, Spinner, TextInput } from 'components/CustomControls';
...
const LabTestCreateForm = props => {
  ...
  const [date] = useState(() => {
    ...
    const currentTime = getTimeFromDate(_dateTime);
    ...
    if (currentTime >= fromTime && currentTime <= toTime) {
      const prevDate = getNewDateBefore(_dateTime, 1);
      return prevDate;
    } else {
      return _dateTime;
    }
  ...
  useEffect(() => {
    const fetchDependencies = async () => {
      try {
        const labShiftReq = http.get(LAB_SHIFT.get_current_shift, { params: { CurrentTime: time24(date) } });
        const testSampleReq = http.get(TEST_SAMPLE.get_all);

        const [labShiftRes, testSampleRes] = await axios.all([labShiftReq, testSampleReq]);
        if (labShiftRes.data.succeeded && testSampleRes.data.succeeded) {
          const labshift = labShiftRes.data.data;

          const testSamples = testSampleRes.data.data.map(sample => ({
            ...sample,
            rowId: uuid(),
            rvP_psi: '',
            colour_ASTM: '',
  ...
  const onSubmit = async e => {
    ...
    const data = {
      date: serverDate(currentdatetime),
      time: floorTime,
      shiftId: currshift.id,
      shiftName: currshift.shiftName,
  ...
  return (
    <PageContainer heading="Lab Test (Create)">
      <FormWrapper>
        <Form>
...
export default LabTestCreateForm;
