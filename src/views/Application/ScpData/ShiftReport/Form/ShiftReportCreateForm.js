...
import { getSign, sleep, stringifyConsole } from 'utils/commonHelper';
import { getFloorTime, getNewDateBefore, getTimeFromDate, serverDate, time24 } from 'utils/dateHelper';
...
const ShiftReportCreateForm = () => {
  ...
  const [currentLabShift, setCurrentLabShift] = useState(null);
  ...
  const getShiftWithReading = (shiftId, date) => {
    const queryParam = {
  ...
  const getAllDependencies = useCallback(() => {
    ...
    Axios.all([
      ...
      http.get(LAB_SHIFT.get_current_shift, { params: { CurrentTime: time24(date) } })
    ]).then(
      Axios.spread((...responses) => {
        ...
        const currentLabShiftResponse = responses[6];
        ...
        if (
          ...
          currentLabShiftResponse.data.succeeded
        ) {
          getShiftWithReading(currentLabShiftResponse.data.data.id, date);
          ...
          setCurrentLabShift(currentLabShiftResponse.data.data);
  ...
  const getBox10Data = () => {
    const shiftReportSettings = shift.find(item => item.shiftSection === Box_10);
  ...
  const getBox11Data = () => {
    const shiftReportSettings = shift.find(item => item.shiftSection === Box_11);
  ...
  //#region Effects
  // useEffect(() => {
  //   const currentTime = getTimeFromDate(datex);

  //   const fromTime = getTimeFromDate(new Date(`0000-01-01 00:00:01`));
  //   const toTime = getTimeFromDate(new Date(`0000-01-01 06:59:59`));

  //   if (currentTime >= fromTime && currentTime <= toTime) {
  //     const prevDate = getNewDateBefore(datex, 1);
  //     setDatex(prevDate);
  //   }
  // }, [datex]);
  ...
  const onSubmit = async e => {
    ...
    let postingDateTime = new Date();
    const currentTime = getTimeFromDate(postingDateTime);
    const fromTime = getTimeFromDate(new Date(`0000-01-01 00:00:01`));
    const toTime = getTimeFromDate(new Date(`0000-01-01 06:59:59`));

    if (currentTime >= fromTime && currentTime <= toTime) {
      const prevDate = getNewDateBefore(postingDateTime, 1);
      postingDateTime = prevDate;
    }
    const shiftEndTime = getTimeFromDate(new Date(`0000-01-01 ${currentLabShift.toTime}`));
    ...
    const { FEED: box10feed, FLASH_OIL: box10flashOil } = getBox10Data();
    ...
    try {
      ...
      if (err?.response?.status === 400) {
        toastAlerts('warning', err?.response?.data?.Message);
      } else {
  ...
  return (
    <PageContainer heading="Shift Report (Create)">
      <FormWrapper>
        <Form>
                      ...
                      .map((x, i) => {
                        return (
                          <TableRow key={x.id}>
                            ...
                            <TableCell align="center">{getBox10Data()[x.calculationType]['m3']}</TableCell>
                            <TableCell align="center">{getBox10Data()[x.calculationType]['mt']}</TableCell>
                            <TableCell align="center">{getBox10Data()[x.calculationType]['percent']}</TableCell>
                          </TableRow>
                        );
                      })}
                  </TableBody>
                      ...
                      .map((x, i) => {
                        return (
                          <TableRow key={x.id}>
                            ...
                            <TableCell align="center">{getBox11Data()[x.calculationType]['m3']}</TableCell>
                            <TableCell align="center">{getBox11Data()[x.calculationType]['mt']}</TableCell>
                            <TableCell align="center">{getBox11Data()[x.calculationType]['percent']}</TableCell>
                          </TableRow>
                        );
                      })}
                  </TableBody>
