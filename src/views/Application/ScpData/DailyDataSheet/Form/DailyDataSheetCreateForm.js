/**
 * Title: Daily datasheet create form (reformed)
 * Description:
 * Author: Nasir Ahmed
 * Date: 16-March-2022
...
import { fillDropDown, getSign, stringifyConsole } from 'utils/commonHelper';
...
const DailyDataSheetCreateForm = () => {
  ...
  useEffect(() => {
    const fetchDataSheetSettingsWithReading = async () => {
      ...
      const dailyDataSheetReadingReq = http.get(DAILY_DATA_SHEET_SETTINGS.get_data_sheet_with_reading, {
        params: { date: serverDate(new Date()) }
      });
      try {
        ...
        if (tanksRes.data.succeeded && dailyDatasheetReadingRes.data.succeeded) {
          const settings = dailyDatasheetReadingRes.data.data.map((ddssec, souceIndex, sourceArr) => {
  ...
  return (
    <PageContainer heading="Daily Data Sheet (Create)">
      <div>
