...
import { DECOKING_PARAMETERS } from 'constants/ApiEndPoints/v1';
...
const DecokingParamerDetails = props => {
  ...
  useEffect(() => {
    if (itemKey) {
      http
        .get(`${DECOKING_PARAMETERS.get_single}/${itemKey}`)
        .then(res => {
      ...
      <Table className={classes.table} aria-label="custom pagination table">
        <TableBody>
          <TableRow>
            ...
            <TableCell align="left">{`: ${details.parameterName} (${details.operationGroup.groupName})`}</TableCell>
          </TableRow>
...
export default DecokingParamerDetails;
