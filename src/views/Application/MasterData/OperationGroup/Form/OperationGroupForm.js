...
const OperationGroupForm = props => {
  ...
  const validate = (fieldValues = state) => {
    ...
    if ('groupName' in fieldValues) {
      ...
      } else if (fieldValues.groupName.length > 50) {
        temp.groupName = 'Group Name can not exceed fifty character';
      } else {
        temp.groupName = '';
      }
    }
    if ('alias' in fieldValues) {
      ...
      } else if (fieldValues.alias.length > 6) {
        temp.alias = 'Group Name can not exceed six character';
      } else {
        temp.alias = '';
      }
    }
